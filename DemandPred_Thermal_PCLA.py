...
def split_dataset(df, timesteps):
    ...
    df_test = df.loc['2020-04-01 00:00:00':'2021-02-26 00:00:00', :]
    ...
    scaled_features = scalerX.transform(df_testX.values)
    df_testX = pd.DataFrame(scaled_features, index=df_testX.index, columns=df_testX.columns)
    df_testY = pd.DataFrame()
    df_testY['LOAD'] = df_test['LOAD']
    ...
    dataX, dataY, testDT = [], [], []
    for k in range(0, len(df_testY) - timesteps - 25):
        _x = df_testX.iloc[k:k + timesteps].values.tolist()
        _y = df_testY.iloc[k + timesteps + 25].values.tolist()
        _dt = df_testY.index[k + timesteps + 25]
        dataX.append(_x)
        dataY.append(_y)
        testDT.append(_dt)
